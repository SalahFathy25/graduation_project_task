...
class _TripItineraryPageState extends State<TripItineraryPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _downloadOfflineMap(ll.LatLng center, AppLocalizations l10n) async {
    ...
    // In a real scenario with flutter_map_cache, tiles are cached automatically.
    // Here we simulate the process for the user feedback.
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l10n.localeName == 'ar' ? 'تم حفظ بيانات الخرائط للاستخدام بدون إنترنت' : 'Map data saved for offline use'),
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          ...
                          style: GoogleFonts.almarai(color: AppColors.primary, fontSize: 11, fontWeight: FontWeight.bold),
                        ),
                      ),
                      Text(
                        (activity['startTime'] ?? '10:00').toString(),
                        style: GoogleFonts.almarai(color: AppColors.primary, fontWeight: FontWeight.bold, fontSize: 12),
                      ),
                    ],
                  ),
